...
while True:
  ...
  if episode_number>iteration:
    ...
    if done:
      ...
      running_sum.append(reward_sum)
      running_mean.append(running_reward)
      reward_sum = 0
      observation = env.reset()
      prev_x = None
    if reward != 0:
      vector_reward.append(reward)


      print ('Episode %d Result: ' % episode_number + 'Defeat!' if reward == -1 else 'VICTORY!')
runmean=np.cumsum(running_mean)
runsum=np.cumsum(running_sum)
cumulative_reward=np.cumsum(vector_reward)
# plt.plot(cumulative_reward,c='blue')
plt.plot(running_mean,c='red')
plt.plot(running_sum,c='green')
plt.show()
